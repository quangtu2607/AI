...
def cornersHeuristic(state, problem):
    ...
    if all(corn == 1 for corn in list(state[1])):
        return 0

    for corn in list(state[1]):
        if corn == 0:
            remCorn.append(corners[list(state[1]).index(corn)])
    for corn in remCorn:
        distance += mazeDistance(curState, corn, problem.gameState)
        curState = corn
